...
/* ---------------------------------------------------------------------- */
/* AVR    Pololu Orangutan SVP  (Atmega1284 @ 20 MHz) */
/* ---------------------------------------------------------------------- */
...
/* Set up AVR 10kHz clock driver interrupt using TIMER2 */
/*
 * TIMER3 Description:    The pre-scaler and the timer count divide the timer-counter
 *                 clock frequency to give a timer overflow interrupt rate:
 *
 *                Interrupt rate =  20MHz / (prescaler * (65535 - TCNT2))
 *
 *                TCCR3B[b2:0]   Prescaler    Freq [KHz], Period [usec] after prescale
 *                0x0            (TC stopped)     0         0
 *                0x1                1        16000.        0.0625
 *                0x2                8         2000.        0.500
 *                0x3               64          500.        2.000
 *                0x4              256          250.        4.000
 *                0x5             1024          125.        8.000
 *                0x6             External clock source on Tn pin. Clock on falling edge.
 *                0x7             External clock source on Tn pin. Clock on rising edge.
 */
 
/* Note: COUNT is 63535 for 10kHz, (20000000/(1 * (65535 - COUNT))). */

#define INIT_TIMER_COUNT 63535
