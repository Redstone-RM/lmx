 ...
 *  18 Oct 2015 dep Created.  Pololu Orangutan SVP (Atmega1284P)
 *  
 */
/* -------------------------------------- */
...
#if (MACHINE == MACH_SVP)  /* Pololu Orangutan SVP */
...
void printkbuf(char *s) {
   ...
   PRINTF(s);
   #endif
   #if ((MACHINE == MACH_M68K) || (MACHINE == MACH_SVP))
   PRINTF("%s\n",s);
   #endif
}
...
/* -------------------------------------- */
/* 3 leds on Pololu Orangutan SVP pins PA0, PA1, PA2  */
// Not using the SVP's user LEDs, but adding LED+Resistor combinations
// LED with in-line serial resistor  - LED anode connected to PA0 resistor to GND
#if ((MACHINE == MACH_SVP))  
#define LED1_ON  set_digital_output(IO_A0,HIGH)
#define LED1_OFF set_digital_output(IO_A0,LOW)

#define LED2_ON  set_digital_output(IO_A1,HIGH)
#define LED2_OFF set_digital_output(IO_A1,LOW)

#define LED3_ON  set_digital_output(IO_A2,HIGH)
#define LED3_OFF set_digital_output(IO_A2,LOW)
#endif
